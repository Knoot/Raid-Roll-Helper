--[[		new
	фильтр по классу/спеку, ролл "нужно" не доступен, если вы не можете получить данный предмет при текущей специализации(или выбранной добыче для специализации)
	ролл "на трансмог" не доступен если предмет является кольцом, шеей, аксессуаром или если у вас уже есть такая модель
	окно победы в ролле с информацией о выигранном предмете и у кого его можно забрать
	предметы с сокетом теперь считаются как +15 к илвл
	при отправке ролла есть возможность запретить роллить на оффспек или трансмог
	при отправке ролла автоматически ставится или снимается галочка(на трансмог), в зависимости от того, есть ли у вас такая модель
	изменен дизайн
	
	--**
	баг при ролле предметов не из подземелий/рейдов
]]

--[[		тз
?	-	+15 к илвл при наличии EMPTY_SOCKET_PRISMATIC	GetItemStats
	-	не 100% срабатывание кнопок	*
