...
frame:SetWidth(180)
frame:SetLabel(L["Оповещение о победе"])
frame:SetTooltip(L["Показывать окно с оповещением о победе"])
...
frame:SetTooltip(L["При получении добычи показывать окно с предложением выставить предмет на ролл"])
frame:SetCallback('OnValueChanged', function()
